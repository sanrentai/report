...
//        this.version = 0;
    ...
    // 员工表
    @JoinColumn(name = "ins_rep", referencedColumnName = "EMP_ID", insertable = false, updatable = false)
    @ManyToOne(fetch = FetchType.LAZY)
    private Employee insUserInfo;

    public Employee getInsUserInfo() {
        return insUserInfo;
    }

    public void setInsUserInfo(Employee insUserInfo) {
        this.insUserInfo = insUserInfo;
    }

    // 修改人
    @JoinColumn(name = "upd_rep", referencedColumnName = "EMP_ID", insertable = false, updatable = false)
    @ManyToOne(fetch = FetchType.LAZY)
    private Employee updUserInfo;

    public Employee getUpdUserInfo() {
        return updUserInfo;
    }

    public void setUpdUserInfo(Employee updUserInfo) {
