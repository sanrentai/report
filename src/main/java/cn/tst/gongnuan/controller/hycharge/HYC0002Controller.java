 ...
 */
@ViewScoped
@Named(SepC.ControllerID.HYC0002)
public class HYC0002Controller extends BusinessBaseController {

    private static final Logger LOG = Logger.getLogger(HYC0002Controller.class.getName());

    @EJB
    private HYC0002BizLogic bizLogic;

    /**
     * 物资信息查看视图模型
     */
    private HYC0002ViewModel vm;

    @PostConstruct
    public void init() {
        vm = new HYC0002ViewModel();
        bizLogic.loadHYC0002ViewModel(vm);
    }
    ...
    public void createRatioModel() {
        ...
        for(HySouFeiNianDuBiaoByGongSiDTO item: vm.getShuJuList()) {
    ...
    //                        Getter Setter
    //*****************************************************************
    public HYC0002ViewModel getVm() {
        return vm;
    }

    public void setVm(HYC0002ViewModel vm) {
        this.vm = vm;
    }
